...
harness/  Crash-Harness: SIGKILL an jedem I/O-Punkt, in CI            [fertig]
          dm-dust und dm-flakey: Lesefehler, Korruption, in CI      [fertig]
          Absturz im degradierten Betrieb: entschieden, gemessen   [fertig]
engine/   Planung von Schreibpfad und Rebuild — kein I/O            [fertig]
          ...
          Doppelausfall: zwei Data-Slots über P+Q                   [fertig]
...
ctl/      `ferrite create`, `status` und `run` — kein Daemon        [fertig]
          gRPC-Daemon, Web-UI, Scrub- und Rebuild-Kommandos          [offen]
```
